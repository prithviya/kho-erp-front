...
import ventorService from '../../services/ventor.service';
import ventorServiceService from '../../services/ventorService.service';
...
const VendorManagement = () => {
    ...
    const [selectedAddCategories, setSelectedAddCategories] = useState([]);
    ...
    const normalizeService = (rawService) => {
        const serviceId = rawService?.serviceId ?? rawService?.service_id ?? rawService?.service_type ?? rawService?.serviceType ?? rawService?.serviceName ?? rawService?.service_name ?? rawService?.name;
        const details = rawService?.details ?? rawService?.serviceDetails ?? {};
        ...
        return {
            recordId: rawService?.id ?? rawService?.vserid ?? rawService?.vendorServiceId,
            serviceId: String(serviceId || 'service'),
            serviceName: rawService?.serviceName ?? rawService?.service_name ?? rawService?.name ?? String(serviceId || 'Service'),
            details: parsedDetails,
            price: rawService?.price ?? rawService?.cost ?? rawService?.amount ?? rawService?.service_cost ?? rawService?.perpostcost ?? rawService?.perpagecost ?? rawService?.perdaycost ?? 0,
            perPageCost: rawService?.perPageCost ?? rawService?.perpagecost ?? 0,
            perPostCost: rawService?.perPostCost ?? rawService?.perpostcost ?? 0,
            perDayCost: rawService?.perDayCost ?? rawService?.perdaycost ?? 0
        };
    };

    const normalizeVendor = (rawVendor, services = []) => {
        const vendorId = rawVendor?.id ?? rawVendor?.vid ?? rawVendor?.vendorId;
        const serviceRows = services.map(normalizeService);
        const selectedServices = rawVendor?.selectedServices || serviceRows.map(service => service.serviceId);
        const serviceDetails = { ...rawVendor?.serviceDetails };
        const servicePricing = { ...rawVendor?.servicePricing };

        serviceRows.forEach(service => {
            serviceDetails[service.serviceId] = service.details || {};
            servicePricing[service.serviceId] = { price: service.price };
        });

        return {
            ...rawVendor,
            id: vendorId,
            name: rawVendor?.name ?? rawVendor?.vendorName ?? rawVendor?.ventor_name ?? '',
            email: rawVendor?.email ?? rawVendor?.ventor_email ?? '',
            phone: rawVendor?.phone ?? rawVendor?.ventor_phone ?? '',
            companyName: rawVendor?.companyName ?? rawVendor?.company_name ?? rawVendor?.ventor_company ?? '',
            selectedServices,
            serviceDetails,
            servicePricing,
            serviceRows,
            totalCost: rawVendor?.totalCost ?? getTotalCost(selectedServices, servicePricing),
            status: rawVendor?.status ?? 'Active'
        };
    };
    ...
    const fetchVendors = async () => {
        ...
        setLoadError('');
        try {
            const [vendorResponse, serviceResponse, catalogResponse] = await Promise.all([
                ventorService.getVendors(),
                ventorServiceService.getAll(),
                leadService.getCategoriesWithServices().catch(() => ({ data: [] }))
            ]);
            const catalogData = getResponseData(catalogResponse);
            const catalogServices = Array.isArray(catalogData)
                ? catalogData.flatMap(category => category.services || [category])
                : [];
            setServiceCatalog(catalogServices);
            if (Array.isArray(catalogData) && catalogData.length > 0) {
                const categoryIcons = { dm: Megaphone, designer: Palette, website: Globe, seo: TrendingUp, smm: Image, ads: TrendingUp, webapp: Code };
                const normalizedCategories = catalogData.map((category, categoryIndex) => {
                    const categoryId = String(category.id ?? category.categoryId ?? category.code ?? category.name ?? categoryIndex);
                    const categoryName = category.name ?? category.categoryName ?? category.label ?? `Category ${categoryIndex + 1}`;
                    return {
                        id: categoryId,
                        label: categoryName,
                        icon: categoryIcons[category.code] || categoryIcons[categoryId] || Layers,
                        color: defaultServiceCategories[categoryIndex % defaultServiceCategories.length].color,
                        services: (category.services || []).map((service, serviceIndex) => ({
                            id: String(service.id ?? service.serviceId ?? service.code ?? service.name ?? serviceIndex),
                            name: service.name ?? service.serviceName ?? service.label ?? `Service ${serviceIndex + 1}`,
                            hasDetails: true,
                            icon: FileText,
                            pricingType: service.pricingType ?? service.pricing_type ?? service.costType ?? service.cost_type
                        }))
                    };
                });
                setServiceCategories(normalizedCategories);
                setSelectedAddCategories([]);
                setSelectedEditCategories([]);
            }
            const serviceList = getResponseData(serviceResponse);
            const servicesByVendor = serviceList.reduce((groups, service) => {
                const vendorId = service?.vid ?? service?.vendorId ?? service?.vendor_id;
                if (vendorId !== undefined) groups[vendorId] = [...(groups[vendorId] || []), service];
                return groups;
            }, {});
            const vendorList = getResponseData(vendorResponse);
            const groupedVendors = vendorList.reduce((groups, vendor) => {
                const vendorId = vendor?.id ?? vendor?.vid ?? vendor?.vendorId;
                groups[vendorId] = [...(groups[vendorId] || []), vendor];
                return groups;
            }, {});
            setVendors(Object.values(groupedVendors).map(rows => {
                const vendor = rows[0];
                const vendorId = vendor?.id ?? vendor?.vid ?? vendor?.vendorId;
                const joinedServices = rows.filter(row => row.service_name || row.service_type);
            ...
            setLoadError(error.message || 'Failed to load vendors');
    ...
    const getPricingType = (serviceId) => {
        const service = serviceCategories
            .flatMap(category => category.services)
            .find(item => String(item.id) === String(serviceId));
        const catalogService = serviceCatalog.find(item => String(item.id ?? item.serviceId ?? item.service_id) === String(serviceId));
        const rawType = service?.pricingType ?? catalogService?.pricingType ?? catalogService?.pricing_type;
        if (rawType) {
            const normalizedType = String(rawType).toLowerCase().replace(/[-\s]/g, '_');
            return ({ perpage: 'per_page', perpost: 'per_post', perday: 'per_day' }[normalizedType] || normalizedType);
        }

        const serviceName = String(service?.name ?? catalogService?.name ?? '').toLowerCase();
        if (serviceName.includes('shoot')) return 'per_day';
        if (serviceName.includes('poster') || serviceName.includes('video') || serviceName.includes('reel')) return 'per_post';
        if (serviceName.includes('website')) return 'per_page';
        return 'fixed';
    };

    const getPricingLabel = (serviceId) => ({
        per_page: 'Per Page Cost (₹)',
        per_post: 'Per Post Cost (₹)',
        per_day: 'Per Day Cost (₹)',
        fixed: 'Cost (₹)'
    }[getPricingType(serviceId)] || 'Cost (₹)');

    const toggleCategory = (categoryId, setSelectedCategories, setForm) => {
        setSelectedCategories(previous => {
            if (previous.includes(categoryId)) {
                const category = serviceCategories.find(item => item.id === categoryId);
                setForm(form => ({
                    ...form,
                    selectedServices: form.selectedServices.filter(serviceId =>
    ...
    const saveVendorServices = async (vendorId, selectedServices, serviceDetails, servicePricing, existingServices = []) => {
        const existingByService = new Map(existingServices.map(service => [service.serviceId, service]));
        await Promise.all(selectedServices.map(async (serviceId) => {
            const payload = {
                vid: vendorId,
                serviceId,
                service_type: serviceId,
                serviceName: getServiceLabel(serviceId),
                service_name: getServiceLabel(serviceId),
                details: serviceDetails[serviceId] || {},
                price: Number(servicePricing[serviceId]?.price || 0),
                perpagecost: getPricingType(serviceId) === 'per_page' ? Number(servicePricing[serviceId]?.price || 0) : 0,
                perpostcost: getPricingType(serviceId) === 'per_post' ? Number(servicePricing[serviceId]?.price || 0) : 0,
                perdaycost: getPricingType(serviceId) === 'per_day' ? Number(servicePricing[serviceId]?.price || 0) : 0
            };
            const existing = existingByService.get(serviceId);
            if (existing?.recordId) {
                return ventorServiceService.update(existing.recordId, payload);
            }
            return ventorServiceService.create(payload);
        }));

        const selectedIds = new Set(selectedServices);
        await Promise.all(existingServices
            .filter(service => service.recordId && !selectedIds.has(service.serviceId))
            .map(service => ventorServiceService.delete(service.recordId)));
    };

    const renderServiceDetailsForm = (serviceId, details, onChange) => {
        const service = getServiceDetails(serviceId);
    ...
    const handleSubmit = async (e) => {
        ...
        setIsSaving(true);
        try {
            const response = await ventorService.create({
                name: formData.name,
                ventor_name: formData.name,
                email: formData.email,
                ventor_email: formData.email,
                phone: formData.phone,
                ventor_phone: formData.phone,
                companyName: formData.companyName || '',
                ventor_company: formData.companyName || '',
                status: 'Active',
                totalCost
            });
            const createdVendor = getResponseData(response);
            const vendorId = createdVendor?.id ?? createdVendor?.vid ?? response?.id;
            if (!vendorId) throw new Error('Vendor was created without an ID');
            await saveVendorServices(vendorId, formData.selectedServices, formData.serviceDetails, formData.servicePricing);
    ...
    const handleEdit = (vendor) => {
        ...
        setSelectedEditCategories(serviceCategories
            .filter(category => category.services.some(service => vendor.selectedServices.includes(service.id)))
    ...
    const handleUpdate = async (e) => {
        ...
        setIsSaving(true);
        try {
            await ventorService.update(selectedVendor.id, {
                name: editFormData.name,
                ventor_name: editFormData.name,
                email: editFormData.email,
                ventor_email: editFormData.email,
                phone: editFormData.phone,
                ventor_phone: editFormData.phone,
                companyName: editFormData.companyName || '',
                ventor_company: editFormData.companyName || '',
                totalCost
            });
            await saveVendorServices(selectedVendor.id, editFormData.selectedServices, editFormData.serviceDetails, editFormData.servicePricing, selectedVendor.serviceRows || []);
    ...
    const getServiceLabel = (serviceId) => {
        ...
        const catalogService = serviceCatalog.find(item => String(item.id ?? item.serviceId ?? item.service_id) === String(serviceId));
        if (catalogService?.name) return catalogService.name;
        const serviceRow = vendors.flatMap(vendor => vendor.serviceRows || []).find(row => row.serviceId === serviceId);
        return serviceRow?.serviceName || serviceId;
    };
    ...
    const vendorServices = vendors.flatMap(vendor => (vendor.serviceRows || []).map(service => ({
        ...service,
        ...
        <div className="min-h-screen bg-gray-50 py-4 px-4 sm:px-6 lg:px-8">
            <div className="">
                ...
                <div className="bg-white rounded-xl shadow-lg overflow-hidden">
                    ...
                    {!isLoading && loadError && (
                        <div className="border-t border-red-100 bg-red-50 px-4 py-4 text-center text-sm text-red-700">
                            <p>{loadError}</p>
                            <button type="button" onClick={fetchVendors} className="mt-2 font-medium underline">
                                Try again
                            </button>
                        </div>
                    )}
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Service Name</th>
                                    <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Cost Type</th>
                                    <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Service Cost</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-200 bg-white">
                                {vendorServices.length > 0 ? vendorServices.map((service, index) => (
                                    <tr key={service.recordId || `${service.vendorName}-${service.serviceId}-${index}`} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-sm text-gray-500">{index + 1}</td>
                                        <td className="px-4 py-3">
                                            <p className="text-sm font-medium text-gray-900">{service.vendorName}</p>
                                            <p className="text-xs text-gray-500">{service.companyName || 'N/A'}</p>
                                        </td>
                                        <td className="px-4 py-3 text-sm font-medium text-blue-700">
                                            {getServiceLabel(service.serviceId)}
                                        </td>
                                        <td className="px-4 py-3 text-sm text-gray-700">
                                            {getPricingLabel(service.serviceId).replace(' (₹)', '')}
                                        </td>
                                        <td className="px-4 py-3 text-sm font-semibold text-gray-900">
                                            {formatCurrency(
                                                getPricingType(service.serviceId) === 'per_page'
                                                    ? service.perPageCost
                                                    : getPricingType(service.serviceId) === 'per_post'
                                                        ? service.perPostCost
                                                        : getPricingType(service.serviceId) === 'per_day'
                                                            ? service.perDayCost
                                                            : service.price
                                            )}
                                        </td>
                                    ...
                                    <tr>
                                        <td colSpan="5" className="px-4 py-8 text-center text-sm text-gray-500">No vendor services found</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
                                            ...
                                            <div className="space-y-4">
                                                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                                                    {serviceCategories.map(category => (
                                                        <label key={category.id} className="flex cursor-pointer items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-700">
                                                            <input
                                                                type="checkbox"
                                                                checked={selectedAddCategories.includes(category.id)}
                                                                onChange={() => toggleCategory(category.id, setSelectedAddCategories, setFormData)}
                                                ...
                                                {serviceCategories.filter(category => selectedAddCategories.includes(category.id)).map(category => {
                                                            ...
                                                            <div key={serviceId} className="overflow-hidden rounded-lg border-2 border-gray-200">
                                                                ...
                                                                <div className="p-3">
                                                                    ...
                                                                    <div className="mt-3">
                                                                        <label className="block text-sm font-medium text-gray-700 mb-1 flex items-center gap-1">
                                                                            <DollarSign size={14} />
                                                                            {getPricingLabel(serviceId)} <span className="text-red-500">*</span>
                                                                        </label>
                                                                        <input
                                                                            type="number"
                                                                            min="0"
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="space-y-4">
                                                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                                                    {serviceCategories.map(category => (
                                                        <label key={category.id} className="flex cursor-pointer items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-700">
                                                            <input
                                                                type="checkbox"
                                                                checked={selectedEditCategories.includes(category.id)}
                                                                onChange={() => toggleCategory(category.id, setSelectedEditCategories, setEditFormData)}
                                                                className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                                                            />
                                                            {category.label}
                                                        </label>
                                                ...
                                                {serviceCategories.filter(category => selectedEditCategories.includes(category.id)).map(category => {
                                                    const Icon = category.icon;
                                                    return (
                                                        <div key={category.id} className={`border rounded-lg p-3 ${getCategoryColor(category.color)}`}>
                                                            <h3 className="text-sm font-medium text-gray-600 mb-2 flex items-center gap-2">
                                    ...
                                    <div className="bg-gray-50 rounded-lg p-4 mt-4">
                                        ...
                                        <div className="space-y-2">
                                            {selectedVendor.selectedServices.map(serviceId => {
                                                const service = getServiceDetails(serviceId) || selectedVendor.serviceRows?.find(row => row.serviceId === serviceId);
                                                    ...
                                                    <div key={serviceId} className="border-b border-gray-200 pb-3 last:border-0">
                                                        <div className="flex justify-between items-center">
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                <span className="text-sm font-medium text-gray-700">{service.name || service.serviceName}</span>

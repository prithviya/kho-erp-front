...
import taskService, { TASK_STATUSES, TASK_PRIORITIES, getStatusMeta, getPriorityMeta } from "../../services/task.service";
...
export default function AssignTask() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [projectRes, userRes, categoryRes] = await Promise.allSettled([
        ...
        userManagementService.getDirectory(),
  ...
  const serviceMap = useMemo(() => {
    ...
    categories.forEach((category) => {
      (category.services || category.Services || []).forEach((service) => map.set(Number(service.id), service));
    });
    return map;
  }, [categories]);
  ...
  const openCreate = () => {
    ...
    setForm({
      ...EMPTY_FORM,
      projectOnboardId: projectFilter || "",
  ...
  const setField = (name, value) => {
    setForm((prev) => {
      ...
      // Service belongs to a project; clear it when the project changes.
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-4 py-4 sm:px-6 lg:px-8">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-gray-500">
            {superAdmin ? "All tasks across projects" : "Tasks you assign or report on"}
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <select value={projectFilter} onChange={(e) => setProjectFilter(e.target.value)} className="ml-auto rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                {["Task", "Project", "Assigned To", "Reporting Head", "Priority", "Due", "Status", "Action"].map((h) => (
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {loading ? (
                <tr><td colSpan={8} className="px-4 py-8 text-center text-sm text-gray-500">Loading tasks...</td></tr>
              ) : tasks.length === 0 ? (
                <tr><td colSpan={8} className="px-4 py-8 text-center text-sm text-gray-500">No tasks found.</td></tr>
              ) : (
                tasks.map((task) => (
                  <tr key={task.id} className="transition-colors hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <p className="text-sm text-gray-800">{task.project?.projectName || "-"}</p>
                      <p className="text-xs text-gray-500">{task.project?.companyName || ""}</p>
                    </td>
                    <td className="px-4 py-3">
                      <span className="rounded-full bg-purple-100 px-2 py-0.5 text-xs text-purple-700">{userName(task.assignee)}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="rounded-full bg-blue-100 px-2 py-0.5 text-xs text-blue-700">{userName(task.reportingHead)}</span>
                    </td>
                    <td className="px-4 py-3"><Badge meta={getPriorityMeta(task.priority)} /></td>
                    <td className="px-4 py-3">
                      <span className={`text-sm ${isOverdue(task) ? "font-medium text-red-600" : "text-gray-700"}`}>{formatDate(task.dueDate)}</span>
                    </td>
                    <td className="px-4 py-3">
                      <select
                        value={task.status}
            ...
            <div className="relative w-full max-w-xl rounded-xl bg-white shadow-xl">
              ...
              <div className="space-y-4 px-6 py-5 text-sm">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div><p className={labelCls}>Priority</p><Badge meta={getPriorityMeta(selectedTask.priority)} /></div>
                  <div><p className={labelCls}>Status</p><Badge meta={getStatusMeta(selectedTask.status)} /></div>
                  <div><p className={labelCls}>Due Date</p><p className={isOverdue(selectedTask) ? "font-medium text-red-600" : "text-gray-800"}>{formatDate(selectedTask.dueDate)}</p></div>
